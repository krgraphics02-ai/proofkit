...
const makeStyles = (dark) => `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    --bg: ${dark ? "#0a0f1a" : "#e8edf5"};
    --surface: ${dark ? "rgba(255,255,255,0.06)" : "rgba(255,255,255,0.7)"};
    --surface2: ${dark ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.5)"};
    --border: ${dark ? "rgba(255,255,255,0.1)" : "rgba(255,255,255,0.8)"};
    --border2: ${dark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)"};
    --orange: #00C27C; --orange-soft: rgba(0,194,124,0.15);
    --green: ${dark ? "#00e676" : "#00a152"}; --green-soft: rgba(0,230,118,0.12);
    --yellow: ${dark ? "#ffd600" : "#f9a825"}; --yellow-soft: rgba(255,214,0,0.12);
    --red: #ff3b5c; --red-soft: rgba(255,59,92,0.12);
    --blue: #4da6ff; --blue-soft: rgba(77,166,255,0.12);
    --gold: #ffab00; --gold-soft: rgba(255,171,0,0.12);
    --text: ${dark ? "rgba(255,255,255,0.92)" : "rgba(0,0,0,0.85)"};
    --text-muted: ${dark ? "rgba(255,255,255,0.35)" : "rgba(0,0,0,0.35)"};
    --nav-bg: ${dark ? "rgba(10,15,26,0.85)" : "rgba(232,237,245,0.85)"};
    --glass: ${dark ? "rgba(255,255,255,0.06)" : "rgba(255,255,255,0.65)"};
    --glass-strong: ${dark ? "rgba(255,255,255,0.1)" : "rgba(255,255,255,0.85)"};
    --shadow: ${dark ? "0 8px 32px rgba(0,0,0,0.4)" : "0 8px 32px rgba(0,0,0,0.08)"};
    --shadow-sm: ${dark ? "0 2px 12px rgba(0,0,0,0.3)" : "0 2px 12px rgba(0,0,0,0.06)"};
    --glow: 0 0 40px rgba(0,194,124,0.15);
  }
  body { background: var(--bg); color: var(--text); font-family: 'Syne', sans-serif; min-height: 100vh; transition: background 0.3s, color 0.3s; }
  body::before { content: ''; position: fixed; inset: 0; background: ${dark ? "radial-gradient(ellipse at 20% 20%, rgba(0,194,124,0.08) 0%, transparent 50%), radial-gradient(ellipse at 80% 80%, rgba(77,166,255,0.06) 0%, transparent 50%)" : "radial-gradient(ellipse at 20% 20%, rgba(0,194,124,0.06) 0%, transparent 50%), radial-gradient(ellipse at 80% 80%, rgba(77,166,255,0.04) 0%, transparent 50%)"}; pointer-events: none; z-index: 0; }

  /* GLASS MIXIN */
  .glass { background: var(--glass); backdrop-filter: blur(20px) saturate(180%); -webkit-backdrop-filter: blur(20px) saturate(180%); border: 1px solid var(--border); box-shadow: var(--shadow); }
  ...
  .login-box { background: var(--glass-strong); backdrop-filter: blur(40px) saturate(200%); -webkit-backdrop-filter: blur(40px) saturate(200%); border: 1px solid var(--border); border-radius: 28px; padding: 40px 36px; width: 100%; max-width: 420px; box-shadow: var(--shadow), var(--glow); }
  .login-logo { display: flex; align-items: center; gap: 10px; margin-bottom: 36px; }
  .login-icon { width: 40px; height: 40px; background: linear-gradient(135deg, #00C27C, #00a366); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 20px; box-shadow: 0 4px 16px rgba(0,194,124,0.4); }
  ...
  .field label { display: block; font-size: 11px; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 8px; font-weight: 600; }
  .field input { width: 100%; background: var(--surface2); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); border: 1px solid var(--border2); border-radius: 14px; padding: 14px 16px; color: var(--text); font-family: 'Syne', sans-serif; font-size: 14px; outline: none; transition: all 0.2s; }
  .field input:focus { border-color: rgba(0,194,124,0.5); background: var(--surface); box-shadow: 0 0 0 3px rgba(0,194,124,0.1); }
  .field input::placeholder { color: var(--text-muted); }
  .login-btn { width: 100%; padding: 15px; background: linear-gradient(135deg, #00C27C, #00a366); border: none; border-radius: 14px; color: #fff; font-family: 'Syne', sans-serif; font-size: 15px; font-weight: 700; cursor: pointer; transition: all 0.2s; margin-top: 8px; box-shadow: 0 4px 20px rgba(0,194,124,0.4); }
  .login-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 28px rgba(0,194,124,0.5); }
  .login-btn:disabled { opacity: 0.5; cursor: not-allowed; transform: none; box-shadow: none; }
  .login-error { background: var(--red-soft); border: 1px solid rgba(255,59,92,0.2); border-radius: 12px; padding: 12px 16px; font-size: 13px; color: var(--red); margin-bottom: 16px; backdrop-filter: blur(10px); }

  /* NAV */
  .app { min-height: 100vh; display: flex; flex-direction: column; position: relative; z-index: 1; }
  .nav { display: flex; align-items: center; justify-content: space-between; padding: 14px 24px; border-bottom: 1px solid var(--border2); position: sticky; top: 0; background: var(--nav-bg); backdrop-filter: blur(30px) saturate(180%); -webkit-backdrop-filter: blur(30px) saturate(180%); z-index: 100; gap: 10px; flex-wrap: wrap; }
  .nav-left { display: flex; align-items: center; gap: 10px; }
  .nav-brand { display: flex; align-items: center; gap: 8px; }
  .brand-icon { width: 30px; height: 30px; background: linear-gradient(135deg, #00C27C, #00a366); border-radius: 9px; display: flex; align-items: center; justify-content: center; font-size: 15px; box-shadow: 0 2px 10px rgba(0,194,124,0.4); }
  .brand-name { font-size: 17px; font-weight: 800; }
  .brand-name span { color: var(--orange); }
  .resto-name-badge { font-size: 12px; color: var(--text-muted); padding: 4px 12px; background: var(--surface2); backdrop-filter: blur(10px); border: 1px solid var(--border2); border-radius: 20px; }
  .role-badge { padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; backdrop-filter: blur(10px); }
  .role-superadmin { background: rgba(255,171,0,0.12); color: var(--gold); border: 1px solid rgba(255,171,0,0.2); }
  .role-manager { background: rgba(77,166,255,0.12); color: var(--blue); border: 1px solid rgba(77,166,255,0.2); }
  .role-employee { background: rgba(0,230,118,0.12); color: var(--green); border: 1px solid rgba(0,230,118,0.2); }
  .nav-right { display: flex; align-items: center; gap: 8px; }
  .user-name { font-size: 13px; color: var(--text-muted); }
  .theme-btn { width: 34px; height: 34px; background: var(--surface2); backdrop-filter: blur(10px); border: 1px solid var(--border2); border-radius: 10px; font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s; }
  .theme-btn:hover { background: var(--surface); }
  .logout-btn { padding: 7px 14px; background: var(--surface2); backdrop-filter: blur(10px); border: 1px solid var(--border2); border-radius: 10px; color: var(--text-muted); font-family: 'Syne', sans-serif; font-size: 12px; cursor: pointer; transition: all 0.2s; }
  .logout-btn:hover { border-color: var(--red); color: var(--red); }
  .nav-tabs { display: flex; background: var(--surface2); backdrop-filter: blur(10px); border-radius: 12px; padding: 3px; gap: 2px; border: 1px solid var(--border2); flex-wrap: wrap; }
  .tab { padding: 7px 14px; border-radius: 9px; border: none; background: transparent; color: var(--text-muted); font-family: 'Syne', sans-serif; font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.2s; white-space: nowrap; position: relative; }
  .tab.active { background: linear-gradient(135deg, #00C27C, #00a366); color: #fff; box-shadow: 0 2px 12px rgba(0,194,124,0.4); }
  .tab-notif { position: absolute; top: 2px; right: 2px; width: 7px; height: 7px; background: var(--red); border-radius: 50%; box-shadow: 0 0 6px rgba(255,59,92,0.6); }
...
@keyframes liquidBubble { 0%{transform:scale(0);opacity:1;} 60%{opacity:0.8;} 100%{transform:scale(1);opacity:0;} }
  ...
  .sa-stat { background: var(--glass); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid var(--border); border-radius: 18px; padding: 20px; text-align: center; box-shadow: var(--shadow-sm); transition: transform 0.2s; }
  .sa-stat:hover { transform: translateY(-2px); }
  ...
  .resto-list { display: flex; flex-direction: column; gap: 10px; margin-bottom: 24px; }
  .resto-card { background: var(--glass); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid var(--border); border-radius: 18px; padding: 18px 20px; display: flex; align-items: center; gap: 14px; transition: all 0.2s; box-shadow: var(--shadow-sm); }
  .resto-card:hover { transform: translateY(-2px); border-color: rgba(0,194,124,0.3); box-shadow: var(--shadow), var(--glow); }
  .resto-avatar { width: 46px; height: 46px; border-radius: 14px; background: var(--orange-soft); border: 1px solid rgba(0,194,124,0.2); display: flex; align-items: center; justify-content: center; font-size: 22px; flex-shrink: 0; }
  ...
  .resto-tag { font-size: 11px; padding: 3px 10px; border-radius: 20px; backdrop-filter: blur(10px); }
  .tag-active { background: var(--green-soft); color: var(--green); border: 1px solid rgba(0,230,118,0.2); }
  .tag-inactive { background: var(--red-soft); color: var(--red); border: 1px solid rgba(255,59,92,0.2); }
  .tag-users { background: var(--blue-soft); color: var(--blue); border: 1px solid rgba(77,166,255,0.2); }
  .tag-proofs { background: var(--orange-soft); color: var(--orange); border: 1px solid rgba(0,194,124,0.2); }
  .resto-actions { display: flex; gap: 8px; }
  .btn-sm { padding: 8px 14px; border-radius: 10px; font-family: 'Syne', sans-serif; font-size: 12px; font-weight: 600; cursor: pointer; border: 1px solid; transition: all 0.2s; backdrop-filter: blur(10px); }
  .btn-orange { background: var(--orange-soft); border-color: rgba(0,194,124,0.3); color: var(--orange); }
  .btn-orange:hover { background: linear-gradient(135deg, #00C27C, #00a366); color: #fff; border-color: transparent; box-shadow: 0 4px 16px rgba(0,194,124,0.4); }
  .btn-red { background: var(--red-soft); border-color: rgba(255,59,92,0.2); color: var(--red); }
  ...
  .add-resto-form { background: var(--glass); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid var(--border); border-radius: 20px; padding: 24px; box-shadow: var(--shadow-sm); }
  .form-grid-3 { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; margin-bottom: 12px; }
  .form-input { background: var(--surface2); backdrop-filter: blur(10px); border: 1px solid var(--border2); border-radius: 12px; padding: 12px 14px; color: var(--text); font-family: 'Syne', sans-serif; font-size: 13px; outline: none; width: 100%; transition: all 0.2s; }
  .form-input:focus { border-color: rgba(0,194,124,0.4); box-shadow: 0 0 0 3px rgba(0,194,124,0.08); }
  .form-input::placeholder { color: var(--text-muted); }
  .add-btn { width: 100%; padding: 13px; background: linear-gradient(135deg, #00C27C, #00a366); border: none; border-radius: 12px; color: #fff; font-family: 'Syne', sans-serif; font-size: 14px; font-weight: 700; cursor: pointer; margin-top: 4px; box-shadow: 0 4px 16px rgba(0,194,124,0.35); transition: all 0.2s; }
  .add-btn:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(0,194,124,0.45); }
  .add-btn:disabled { opacity: 0.4; cursor: not-allowed; transform: none; box-shadow: none; }

  /* UPLOAD */
  .upload-zone { border: 2px dashed var(--border); border-radius: 20px; padding: 40px 24px; text-align: center; cursor: pointer; transition: all 0.25s; background: var(--glass); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); margin-bottom: 20px; position: relative; overflow: hidden; }
  .upload-zone:hover, .upload-zone.drag { border-color: rgba(0,194,124,0.5); background: var(--orange-soft); box-shadow: var(--glow); }
  ...
  .preview-wrap { position: relative; border-radius: 20px; overflow: hidden; background: var(--glass); backdrop-filter: blur(20px); border: 1px solid var(--border); margin-bottom: 20px; box-shadow: var(--shadow-sm); }
  .preview-img { width: 100%; max-height: 300px; object-fit: contain; display: block; }
  .preview-change { position: absolute; bottom: 12px; right: 12px; padding: 8px 16px; background: rgba(0,0,0,0.6); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.15); border-radius: 10px; color: #fff; font-family: 'Syne', sans-serif; font-size: 12px; cursor: pointer; }
  .send-btn { width: 100%; padding: 16px; background: linear-gradient(135deg, #00C27C, #00a366); border: none; border-radius: 14px; color: #fff; font-family: 'Syne', sans-serif; font-size: 15px; font-weight: 700; cursor: pointer; transition: all 0.2s; margin-bottom: 24px; box-shadow: 0 4px 20px rgba(0,194,124,0.4); }
  .send-btn:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 8px 28px rgba(0,194,124,0.5); }
  .send-btn:disabled { opacity: 0.4; cursor: not-allowed; }
  .loading-bar { width: 100%; height: 3px; background: var(--surface2); border-radius: 3px; overflow: hidden; margin-bottom: 12px; }
  .loading-bar-inner { height: 100%; background: linear-gradient(90deg, #00C27C, #4da6ff); border-radius: 3px; animation: loadbar 1.5s ease-in-out infinite; }
  ...
  /* RECORDS */
  .status-badge { padding: 6px 14px; border-radius: 20px; font-size: 12px; font-weight: 700; backdrop-filter: blur(10px); }
  ...
  .status-alert { background: var(--red-soft); color: var(--red); border: 1px solid rgba(255,59,92,0.2); }
  ...
  .timestamp { font-family: 'Inconsolata', monospace; font-size: 13px; color: var(--green); background: var(--green-soft); padding: 4px 10px; border-radius: 8px; display: inline-block; border: 1px solid rgba(0,230,118,0.15); }
  .filters-row { display: flex; gap: 8px; margin-bottom: 16px; flex-wrap: wrap; }
  .filter-btn { padding: 7px 16px; background: var(--glass); backdrop-filter: blur(10px); border: 1px solid var(--border2); border-radius: 20px; color: var(--text-muted); font-family: 'Syne', sans-serif; font-size: 12px; cursor: pointer; transition: all 0.2s; }
  .filter-btn.active { background: var(--orange-soft); border-color: rgba(0,194,124,0.4); color: var(--orange); box-shadow: 0 2px 10px rgba(0,194,124,0.15); }
  .search-row { display: flex; gap: 10px; margin-bottom: 12px; flex-wrap: wrap; }
  .search-input { flex: 1; min-width: 140px; background: var(--glass); backdrop-filter: blur(20px); border: 1px solid var(--border); border-radius: 12px; padding: 11px 14px; color: var(--text); font-family: 'Inconsolata', monospace; font-size: 14px; outline: none; transition: all 0.2s; }
  .search-input:focus { border-color: rgba(0,194,124,0.4); box-shadow: 0 0 0 3px rgba(0,194,124,0.08); }
  .search-input::placeholder { color: var(--text-muted); font-family: 'Syne', sans-serif; font-size: 13px; }
  .date-input { background: var(--glass); backdrop-filter: blur(20px); border: 1px solid var(--border); border-radius: 12px; padding: 11px 14px; color: var(--text); font-family: 'Inconsolata', monospace; font-size: 13px; outline: none; }
  ...
  .record-item { background: var(--glass); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid var(--border); border-radius: 16px; padding: 14px 18px; display: flex; align-items: center; gap: 14px; cursor: pointer; transition: all 0.2s; box-shadow: var(--shadow-sm); }
  .record-item:hover { transform: translateY(-2px); border-color: rgba(0,194,124,0.3); box-shadow: var(--shadow), var(--glow); }
  .record-item.has-alert { border-color: rgba(255,59,92,0.25); }
  .record-thumb { width: 54px; height: 54px; border-radius: 12px; object-fit: cover; background: var(--surface2); flex-shrink: 0; }
  ...
  .record-author { font-size: 11px; color: var(--blue); background: var(--blue-soft); padding: 2px 8px; border-radius: 6px; border: 1px solid rgba(77,166,255,0.15); }
  .record-status { font-size: 20px; flex-shrink: 0; }
  .empty-state { text-align: center; padding: 60px 24px; color: var(--text-muted); }
  .empty-icon { font-size: 48px; margin-bottom: 16px; }

  /* ALERTS */
  .alert-banner { background: rgba(255,59,92,0.08); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid rgba(255,59,92,0.2); border-radius: 16px; padding: 14px 18px; margin-bottom: 10px; display: flex; align-items: flex-start; gap: 12px; animation: fadeUp 0.4s ease; box-shadow: 0 4px 20px rgba(255,59,92,0.1); }
  ...
  /* TEAM */
  .emp-card { background: var(--glass); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid var(--border); border-radius: 20px; padding: 22px; margin-bottom: 10px; box-shadow: var(--shadow-sm); transition: all 0.2s; }
  ...
  .emp-avatar { width: 44px; height: 44px; border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 20px; font-weight: 800; flex-shrink: 0; }
  .emp-avatar-ok { background: var(--green-soft); color: var(--green); border: 1px solid rgba(0,230,118,0.2); }
  .emp-avatar-warn { background: var(--red-soft); color: var(--red); border: 1px solid rgba(255,59,92,0.2); }
  .emp-name { font-size: 16px; font-weight: 700; }
  .emp-role { font-size: 11px; color: var(--text-muted); margin-top: 2px; }
  .emp-stats-row { display: grid; grid-template-columns: repeat(4,1fr); gap: 8px; }
  .emp-stat { background: var(--surface2); backdrop-filter: blur(10px); border: 1px solid var(--border2); border-radius: 14px; padding: 12px; text-align: center; }
  .emp-stat-num { font-size: 22px; font-weight: 800; }
  .emp-stat-label { font-size: 10px; color: var(--text-muted); margin-top: 3px; text-transform: uppercase; letter-spacing: 0.5px; }
  ...
  .emp-bar-bg { height: 5px; background: var(--surface2); border-radius: 6px; overflow: hidden; }
  .emp-bar-fill { height: 100%; border-radius: 6px; transition: width 0.6s ease; }
  .emp-warning { background: var(--red-soft); border: 1px solid rgba(255,59,92,0.15); border-radius: 12px; padding: 10px 14px; margin-top: 12px; font-size: 13px; color: var(--red); }

  /* ADMIN */
  .stats-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 12px; margin-bottom: 28px; }
  .stat-card { background: var(--glass); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid var(--border); border-radius: 18px; padding: 22px; text-align: center; box-shadow: var(--shadow-sm); transition: all 0.2s; }
  ...
  .user-item { background: var(--glass); backdrop-filter: blur(20px); border: 1px solid var(--border); border-radius: 16px; padding: 14px 18px; display: flex; align-items: center; justify-content: space-between; gap: 12px; transition: all 0.2s; }
  .user-item:hover { border-color: rgba(0,194,124,0.2); }
  .user-avatar { width: 40px; height: 40px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 18px; font-weight: 700; flex-shrink: 0; }
  .avatar-manager { background: var(--blue-soft); color: var(--blue); border: 1px solid rgba(77,166,255,0.2); }
  .avatar-employee { background: var(--green-soft); color: var(--green); border: 1px solid rgba(0,230,118,0.2); }
  .user-info { flex: 1; }
  ...
  .del-btn { padding: 6px 14px; background: var(--red-soft); border: 1px solid rgba(255,59,92,0.2); border-radius: 10px; color: var(--red); font-family: 'Syne', sans-serif; font-size: 12px; cursor: pointer; transition: all 0.2s; backdrop-filter: blur(10px); }
  .del-btn:hover { background: rgba(255,59,92,0.2); }
  .add-user-form { background: var(--glass); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid var(--border); border-radius: 20px; padding: 24px; box-shadow: var(--shadow-sm); }
  .form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 12px; }
  .form-select { background: var(--surface2); backdrop-filter: blur(10px); border: 1px solid var(--border2); border-radius: 12px; padding: 12px 14px; color: var(--text); font-family: 'Syne', sans-serif; font-size: 13px; outline: none; width: 100%; }
  .form-select option { background: ${dark ? "#0a0f1a" : "#fff"}; }

  /* ABONNEMENT */
  .sub-wrap { max-width: 500px; margin: 0 auto; }
  .sub-active { background: linear-gradient(135deg, rgba(0,230,118,0.1), rgba(0,194,124,0.08)); backdrop-filter: blur(20px); border: 1px solid rgba(0,230,118,0.2); border-radius: 20px; padding: 24px; margin-bottom: 24px; display: flex; align-items: center; gap: 16px; box-shadow: 0 4px 20px rgba(0,194,124,0.1); }
  ...
  .sub-card { background: var(--glass); backdrop-filter: blur(30px); -webkit-backdrop-filter: blur(30px); border: 1px solid rgba(0,194,124,0.25); border-radius: 24px; padding: 32px; margin-bottom: 20px; position: relative; overflow: hidden; box-shadow: var(--shadow), var(--glow); }
  .sub-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, #00C27C, #4da6ff, #00C27C); }
  ...
  .sub-feature-icon { width: 22px; height: 22px; background: linear-gradient(135deg, rgba(0,194,124,0.2), rgba(0,194,124,0.1)); border: 1px solid rgba(0,194,124,0.3); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 11px; flex-shrink: 0; color: var(--orange); font-weight: 700; }
  .sub-btn { width: 100%; padding: 16px; background: linear-gradient(135deg, #00C27C, #00a366); border: none; border-radius: 14px; color: #fff; font-family: 'Syne', sans-serif; font-size: 15px; font-weight: 700; cursor: pointer; transition: all 0.2s; box-shadow: 0 4px 20px rgba(0,194,124,0.4); }
  .sub-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 28px rgba(0,194,124,0.5); }
  .sub-note { text-align: center; font-size: 12px; color: var(--text-muted); margin-top: 12px; }
  .sub-inactive-banner { background: rgba(0,194,124,0.08); backdrop-filter: blur(10px); border: 1px solid rgba(0,194,124,0.2); border-radius: 16px; padding: 16px 20px; margin-bottom: 24px; font-size: 14px; display: flex; gap: 12px; align-items: flex-start; }
  .modal-del-btn { width: 100%; margin-top: 16px; padding: 13px; background: var(--red-soft); backdrop-filter: blur(10px); border: 1px solid rgba(255,59,92,0.2); border-radius: 12px; color: var(--red); font-family: 'Syne', sans-serif; font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.2s; }
  .modal-del-btn:hover { background: rgba(255,59,92,0.15); }

  /* RESULT ROW */
  .result-row { display: flex; gap: 8px; align-items: flex-start; padding: 12px 0; border-bottom: 1px solid var(--border2); font-size: 14px; }
  .result-row:last-child { border-bottom: none; }
  .result-row-label { color: var(--text-muted); min-width: 100px; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; padding-top: 2px; font-weight: 600; }
  .result-row-value { color: var(--text); line-height: 1.5; flex: 1; }

  /* MODAL */
  .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.7); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); z-index: 200; display: flex; align-items: center; justify-content: center; padding: 24px; }
  .modal { background: var(--glass-strong); backdrop-filter: blur(40px) saturate(200%); -webkit-backdrop-filter: blur(40px) saturate(200%); border: 1px solid var(--border); border-radius: 24px; max-width: 500px; width: 100%; max-height: 90vh; overflow-y: auto; animation: scaleIn 0.2s ease; box-shadow: var(--shadow), var(--glow); }
  @keyframes scaleIn { from{transform:scale(0.95);opacity:0} to{transform:scale(1);opacity:1} }
  .modal-header { padding: 18px 22px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border2); position: sticky; top: 0; background: var(--glass-strong); backdrop-filter: blur(40px); border-radius: 24px 24px 0 0; }
  .modal-close { width: 32px; height: 32px; background: var(--surface2); border: 1px solid var(--border2); border-radius: 10px; color: var(--text); font-size: 18px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s; }
  .modal-close:hover { background: var(--red-soft); color: var(--red); }
  .modal-img { width: 100%; max-height: 280px; object-fit: contain; background: var(--surface2); }
